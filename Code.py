...
plt.plot(X_normalized, X_b.dot(theta_final), color='red', label='Fitted line')
...
for lr in learning_rates:
    _, cost_histories[lr] = gradient_descent(X_b, Y, theta, lr, iterations)
...
# Performing stochastic and mini-batch gradient descent
theta_sgd, cost_history_sgd = stochastic_gradient_descent(X_b, Y, theta, learning_rate, iterations)
theta_mbgd, cost_history_mbgd = mini_batch_gradient_descent(X_b, Y, theta, learning_rate, iterations, batch_size=20)

# Plotting the cost history of different gradient descent methods
plt.plot(range(1, 51), cost_history[:50], 'b-', label='Batch GD')
plt.plot(range(1, 51), cost_history_sgd[:50], 'green', label='Stochastic GD')
plt.plot(range(1, 51), cost_history_mbgd[:50], 'red', label='Mini-batch GD')
plt.xlabel('Iterations')
plt.ylabel('Cost')
